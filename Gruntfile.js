module.exports = function (grunt) {
    grunt.initConfig({
        ...
        concat: {
            css: {
                src: ["src/css/reset.css", "src/css/components.css", "src/css/main.css", "src/css/type.css"],
        ...
        copy: {
            ...
            js: {
                ...
                options: {
                    process: function(content) {
                        return content.replace(/(\\n[\s]*)/g, "");//.replace(/console\.log\(.+?\);?/g, "");
                    }
                }
            },
            html: {
                src: "temp/index.min.html",
                dest: "dist/public_html/<%= pkg.name %>/index.html"
            },
            server: {
                expand: true,
                cwd: "src/server",
            ...
            other: {
                ...
                dest: "dist/public_html/<%= pkg.name %>/"
            }
        },
        clean: {
            dist: ["dist/company-directory/", "temp"]
        },
        watch: {
            options: {
                spawn: false,
